...
One tiny example is included so you can point real agent code at Bulkhead immediately.

The example can send more than one request. Set `request_count` in [config.example.yaml](/Users/saikrishna/tfy/bulkhead/config.example.yaml) via your local `config.yaml`, or override it with `BULKHEAD_REQUEST_COUNT`.
